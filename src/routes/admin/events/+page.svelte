<script>
  ...
  onMount(async () => {
    try {
      ...
      const data = await getAboutSection(website.id, 'events', true); // Include inactive
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Update content
      const updateSuccess = await updateAboutSection(website.id, 'events', newContent);
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-green-700">
            Konten Events berhasil diperbarui.
          </div>
        </div>
      </div>
    </div>
