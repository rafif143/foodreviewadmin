<script>
  ...
  onMount(async () => {
    try {
      ...
      const data = await getAboutSection(website.id, 'recipe', true); // Include inactive
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Update content
      const updateSuccess = await updateAboutSection(website.id, 'recipe', newContent);
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-green-700">
            Konten Recipe berhasil diperbarui.
          </div>
        </div>
      </div>
    </div>
