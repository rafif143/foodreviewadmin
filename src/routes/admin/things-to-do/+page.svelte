<script>
  ...
  onMount(async () => {
    try {
      ...
      const data = await getAboutSection(website.id, 'things-to-do', true); // Include inactive
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Update content
      const updateSuccess = await updateAboutSection(website.id, 'things-to-do', newContent);
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-green-700">
            Konten Things to Do berhasil diperbarui.
          </div>
        </div>
      </div>
    </div>
