<script>
  ...
  onMount(async () => {
    try {
      ...
      const data = await getAboutSection(website.id, 'cafe', true); // Include inactive
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Update content
      const updateSuccess = await updateAboutSection(website.id, 'cafe', newContent);
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <div class="mt-2 text-sm text-green-700">
            Konten Cafe berhasil diperbarui.
          </div>
        </div>
      </div>
    </div>
